...
const deleteUser = asyncHandler(async (req, res) => {
    ...
    // Does the user still have assigned shippings?
    const shipping = await Shipment.findOne({ user: _id }).lean().exec()
    if (shipping) {
        return res.status(400).json({ message: 'User has assigned shipments' })
    }
